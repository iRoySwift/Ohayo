...
export class FileController {
  ...
  async downloadFile(
    ...
    const filePath = join(process.cwd(), '/dist/apps/public/upload', filename);
    ...
    if (!fileExists) {
      return next(new HttpException('File not found', HttpStatus.NOT_FOUND));
    }
  ...
  async downloadFileStream(
    ...
    readerStream.on('error', () => {
      next(
        new HttpException('下载失败，请重试', HttpStatus.SERVICE_UNAVAILABLE),
      );
    });
  }
  ...
  async downloadBigFile(
    ...
    stat(filePath, (err, stats) => {
      if (err || !stats.isFile()) {
        return next(
          new HttpException('文件不存在或无法访问', HttpStatus.NOT_FOUND),
        );
      }
